...
namespace CraigslistReader
{

    public class CLFeeds
    {
    ...
    public class CLFeedList
    {
        public string Title { get; set; }
        public string Description { get; set; }        
        public bool IsPubDate { get; set; }
        public List<CLFeedItem> itemList { get; set; }
        public CLFeedList()
        {
            Title = "N/A";
            Description = "N/A";          
            IsPubDate = false;
            itemList = new List<CLFeedItem>();
        }
    }
    ...
    public class RSS
    {
        ...
        public static void GetXMLFeedAsList(string rssUrl, int FeedId, int FeedType)
        {
            try
            {
                ...
                if (FeedList != null)
                {
                    List<CLFeedItem> itemLists = FeedList.itemList;
                    if (itemLists != null && itemLists.Count > 0)
                    {
                        ...
                        decimal PollTime = Math.Round(Convert.ToDecimal(ts.TotalMilliseconds), 2);
                        int Counter = 0;
                        foreach (CLFeedItem row in itemLists)
                        {
                            row.FeedId = FeedId;
                            row.InsertTime = DateTime.Now;
                            if (Counter > 0)
        ...
        public static CLFeedList ParseCraiglistDocument(XmlDocument doc)
        {
            var objCraiglistFeed = new CLFeedList();
            bool IsPubDate = false;
            ...
            foreach (XmlNode node in nodeLists)
            {
                foreach (XmlNode chielNode in node.ChildNodes)
                {
                    ...
                    {
                        CLFeedItem item = new CLFeedItem();
        ...
        public static CLFeedList ParseAtomDocument(XmlDocument doc)
        {
            var objCraiglistFeed = new CLFeedList();
            bool IsPubDate = false;
        ...
        public static CLFeedList ParseXMLDocument(XmlDocument doc)
        {
            var objCraiglistFeed = new CLFeedList();
            bool IsPubDate = false;
